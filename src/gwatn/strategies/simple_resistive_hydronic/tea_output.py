...
import satn.dev_utils.price.regp_sync_100_handler as regp_sync_100_handler
...
from satn.strategies.heatpumpwithbooststore.edge import (
    Edge__HeatpumpWithBoostStore as Edge,
)
...
from satn.strategies.heatpumpwithbooststore.node import (
    Node__HeatpumpWithBoostStore as Node,
)
from satn.strategies.heatpumpwithbooststore.tea_config import TeaParams

import gwatn.conversion_factors as cf
from gwatn.enums import RecognizedCurrencyUnit


OUTPUT_FOLDER = "src/satn/strategies/heatpumpwithbooststore/output_data"
...
def export_xlsx(
    tea_params: TeaParams, flo: Flo__HeatpumpWithBoostStore, export_graph: bool
):
    file = OUTPUT_FOLDER + "/result_{}_{}_{}_{}.xlsx".format(
        flo.home_city,
        flo.flo_start_utc.year,
    ...
    start_idx = int(flo.params.StorageSteps / 2)

    # Add to gsr for more blank rows
    gsr = 30
    w = export_best_path_info(flo=flo, workbook=workbook, start_idx=start_idx)
    if export_graph:
        export_flo_graph(
            flo=flo,
            workbook=workbook,
    ...
    export_params_xlsx(tea_params=tea_params, flo=flo, workbook=workbook)
    workbook.close()


def export_best_path_info(
    flo: Flo__HeatpumpWithBoostStore,
    workbook: xlsxwriter.workbook.Workbook,
    start_idx: int,
):
    w = workbook.add_worksheet(f"start {100 * start_idx / flo.params.StorageSteps}%")
    w.freeze_panes(0, 2)
    ...
    if flo.params.CurrencyUnit == RecognizedCurrencyUnit.GBP:
    ...
    w.write(0, 0, "GNode Alias: millinocket.moss", title_format)
    ...
    node: Node = flo.node[0][start_idx]
...
def export_params_xlsx(
    ...
    w.write("B5", flo.params.HouseWorstCaseTempF)
    ...
    w.write("B14", tea_params.system_max_heat_output_swt_f)
    ...
    if flo.params.DistPumpFeedbackModel == ShDistPumpFeedbackModel.ConstantDeltaT:
    ...
    annual_kwh = tea_params.annual_hvac_kwh_th
    ...
    w.write("D12", "SistPumpFeedbackModel", bold)
    w.write("E12", flo.params.DistPumpFeedbackModel.value)
    ...
    file = weather_forecast_sync_100_handler.csv_file_by_uid(flo.params.WeatherUid)
    w.write("A28", "LocalWeatherFile", bold)
    w.write("B28", file)
    w.write("A29", "WeatherUid", bold)
    w.write("B29", flo.params.WeatherUid)

    w.write("A30", "LocalRtPriceFile", bold)
    w.write(
        "B30",
        eprt_sync_100_handler.csv_file_by_uid(price_uid=flo.params.RtElecPriceUid),
    )

    w.write("A31", "RtElecPriceUid", bold)
    w.write("B31", flo.params.RtElecPriceUid)

    w.write("A32", "LocalDistPriceFile", bold)
    w.write(
        "B32", distp_sync_100_handler.csv_file_by_uid(price_uid=flo.params.DistPriceUid)
    )
    w.write("A33", "DistPriceUid", bold)
    w.write("B33", flo.params.DistPriceUid)

    w.write("A35", "Heat Profile", bold)
    w.write("B35", tea_params.scaled_heat_profile_csv)
