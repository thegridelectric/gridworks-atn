...
def atn_params_from_alias(alias: str, now_ms: int) -> AtnParams:
    ...
    with open(f"{EVENTSTORE_DIR}/{file_name}") as f:
        data = json.load(f)
    if "AtnParamsTypeName" not in data.keys():
        raise Exception(f"{file_name} is not an AtnParamsReport")
    if data["AtnParamsTypeName"] != "atn.params.heatpumpwithbooststore":
        raise Exception(
            f"{file_name} doesn't have AtnParams for Heatpumpwithbooststore"
        )
    params = AtnParams_Maker.dict_to_tuple(data["Params"])
    params_report = AtnParamsReport_Maker.dict_to_tuple(data)
    params_report.Params = params
    if params_report.GNodeAlias != alias:
        raise Exception(
            f"file {file_name} was for {params_report.GNodeAlias}",
            f"instead of {alias}",
        )
    if params_report.AtnParamsTypeName != "atn.params.heatpumpwithbooststore":
        raise Exception(
            f"Expects atn.params.heatpumpwithbooststore, got {params_report.AtnParamsTypeName}"
        )

    return typing.cast(AtnParams, params_report.Params)
...
def get_flo_params(
    ...
    defaults = []
    with open(f"{HEAT_PROFILE_FILE}") as f:
        reader = csv.reader(f)
        for row in reader:
            defaults.append(row)
    ...
    try:
        ...
        LOGGER.warning(
            f"No heat profile data found for {alias}. Using heat profile for rose"
        )
        idx = 10
    ...
    d = dict(atn_params.dict(), TypeName="flo.params.heatpumpwithbooststore")
    d["SystemMaxHeatOutputKwAvg"] = strategy_utils.get_system_max_heat_output_kw_avg(
        system_max_heat_output_gpm=atn_params.SystemMaxHeatOutputGpm,
        system_max_heat_output_delta_temp_f=atn_params.SystemMaxHeatOutputDeltaTempF,
    )
    d["K"] = strategy_utils.get_k(
        system_max_heat_output_delta_temp_f=atn_params.SystemMaxHeatOutputDeltaTempF,
        system_max_heat_output_gpm=atn_params.SystemMaxHeatOutputGpm,
        system_max_heat_output_swt_f=atn_params.SystemMaxHeatOutputSwtF,
        room_temp_f=atn_params.RoomTempF,
    )
