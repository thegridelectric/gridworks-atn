...
class GNodeInstance:
    ...
    def __init__(
        ...
        if strategy.value not in AtnSpaceheatStrategyName.values():
            if strategy.value != "AtnHeatPumpWithBoostStore":
                raise DcError(
                    f"strategy {strategy.value} must belong to "
                    f"AtnSpaceheatStrategyName.values(): {AtnSpaceheatStrategyName.values()} "
                )
