"""MessageMaker for r.gnode.distp.sync.req.1_0_0"""
...
from gwatn.types.gnode_distprequest_ps.r_gnode_distp_sync_req.r_gnode_distp_sync_req_1_0_0_payload import (
    Payload,
)
...
class R_Gnode_Distp_Sync_Req_1_0_0(HackTypeBase):
    mp_alias = "r.gnode.distp.sync.req.1_0_0"
    routing_key_base = "gnode.distprequest.ps"

    @classmethod
    def create_payload_from_camel_dict(cls, d: dict) -> Payload:
        if "MpAlias" not in d.keys():
            d["MpAlias"] = "r.gnode.distp.sync.req.1_0_0"
    ...
    def __init__(
        ...
        if agent is None:
            raise Exception(
                f"Message protocol {R_Gnode_Distp_Sync_Req_1_0_0.mp_alias} must be generated by a message agent"
            )
        if agent == test_dummy.TEST_DUMMY_AGENT:
            ...
            if not agent.gni.g_node.is_g_node:
                raise Exception(
                    f"Message protocol {R_Gnode_Distp_Sync_Req_1_0_0.mp_alias} must come from a GNode"
                )
        ...
        p = Payload(
            MpAlias=R_Gnode_Distp_Sync_Req_1_0_0.mp_alias,
