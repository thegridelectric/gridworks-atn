"""PayloadBase for r.gnode.eprt.sync.req.1_0_0"""
import datetime
...
class PayloadBase(NamedTuple):
    ...
    PriceUid: Optional[str] = None
    WorldInstanceAlias: Optional[str] = None
    MpAlias: str = "r.gnode.eprt.sync.req.1_0_0"
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "r.gnode.eprt.sync.req.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of r.gnode.eprt.sync.req.1_0_0, not {self.MpAlias}."
            )
        ...
        if not isinstance(self.TotalSlices, int):
            is_valid = False
            errors.append(f"TotalSlices {self.TotalSlices} must have type int.")
        ...
        if not property_format.is_recognized_price_method(self.MethodAlias):
            is_valid = False
            errors.append(
                f"MethodAlias {self.MethodAlias} must have format PriceMethod."
            )
