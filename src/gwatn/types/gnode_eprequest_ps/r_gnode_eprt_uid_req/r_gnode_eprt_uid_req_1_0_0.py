"""MessageMaker for r.gnode.eprt.uid.req.1_0_0"""
...
from gwatn.types.gnode_eprequest_ps.r_gnode_eprt_uid_req.r_gnode_eprt_uid_req_1_0_0_payload import (
    Payload,
)
...
class R_Gnode_Eprt_Uid_Req_1_0_0(HackTypeBase):
    mp_alias = "r.gnode.eprt.uid.req.1_0_0"
    routing_key_base = "gnode.eprequest.ps"

    @classmethod
    def create_payload_from_camel_dict(cls, d: dict) -> Payload:
        if "MpAlias" not in d.keys():
            d["MpAlias"] = "r.gnode.eprt.uid.req.1_0_0"
    ...
    def __init__(
        ...
        if agent is None:
            raise Exception(
                f"Message protocol {R_Gnode_Eprt_Uid_Req_1_0_0.mp_alias} must be generated by a message agent"
            )
        if agent == test_dummy.TEST_DUMMY_AGENT:
            ...
            if not agent.gni.g_node.is_g_node:
                raise Exception(
                    f"Message protocol {R_Gnode_Eprt_Uid_Req_1_0_0.mp_alias} must come from a GNode"
                )
        ...
        p = Payload(
            MpAlias=R_Gnode_Eprt_Uid_Req_1_0_0.mp_alias,
