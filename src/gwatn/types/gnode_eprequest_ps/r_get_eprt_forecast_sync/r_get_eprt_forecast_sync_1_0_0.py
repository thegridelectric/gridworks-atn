"""MessageMaker for r.get.eprt.forecast.sync.1_0_0"""
...
from gwatn.types.gnode_eprequest_ps.r_get_eprt_forecast_sync.r_get_eprt_forecast_sync_1_0_0_payload import (
    Payload,
)
...
class R_Get_Eprt_Forecast_Sync_1_0_0(HackTypeBase):
    mp_alias = "r.get.eprt.forecast.sync.1_0_0"
    routing_key_base = "gnode.eprequest.ps"

    @classmethod
    def create_payload_from_camel_dict(cls, d: dict) -> Payload:
        if "MpAlias" not in d.keys():
            d["MpAlias"] = "r.get.eprt.forecast.sync.1_0_0"
    ...
    def __init__(
        ...
        if agent is None:
            raise Exception(
                f"Message protocol {R_Get_Eprt_Forecast_Sync_1_0_0.mp_alias} must be generated by a message agent"
            )
        if agent == test_dummy.TEST_DUMMY_AGENT:
            ...
            if not agent.gni.g_node.is_g_node:
                raise Exception(
                    f"Message protocol {R_Get_Eprt_Forecast_Sync_1_0_0.mp_alias} must come from a GNode"
                )
        ...
        p = Payload(
            MpAlias=R_Get_Eprt_Forecast_Sync_1_0_0.mp_alias,
