"""PayloadBase for r.get.eprt.forecast.sync.1_0_0"""
import datetime
...
class PayloadBase(NamedTuple):
    ...
    MpAlias: str = "r.get.eprt.forecast.sync.1_0_0"
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "r.get.eprt.forecast.sync.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of r.get.eprt.forecast.sync.1_0_0, not {self.MpAlias}."
            )
