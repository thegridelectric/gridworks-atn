"""PayloadBase for r.ps.penpal.1_0_0"""
import datetime
...
class PayloadBase(NamedTuple):
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "r.ps.penpal.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of r.ps.penpal.1_0_0, not {self.MpAlias}."
            )
        ...
        if not isinstance(self.RpcQueueName, str):
            is_valid = False
            errors.append(f"RpcQueueName {self.RpcQueueName} must have type str.")
