...
class PayloadBase(NamedTuple):
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "r.eprt.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of r.eprt.1_0_0, not {self.MpAlias}."
            )
        ...
        if not isinstance(self.MarketSlotDurationMinutes, list):
            is_valid = False
            errors.append(
                f"MarketSlotDurationMinutes {self.MarketSlotDurationMinutes} must have type list."
            )
        else:
            for elt in self.MarketSlotDurationMinutes:
                if not isinstance(elt, int):
                    is_valid = False
                    errors.append(
                        f"Elements of the list MarketSlotDurationMinutes must have type int. Error with {elt}"
                    )
