"""PayloadBase for r.eprt.sync.1_0_0"""
import datetime
...
class PayloadBase(NamedTuple):
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "r.eprt.sync.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of r.eprt.sync.1_0_0, not {self.MpAlias}."
            )
