...
from gwatn.types.epda.gt_epda_actual_1_0_0 import Gt_Epda_Actual_1_0_0
...
from gwatn.types.ps_electricityprices_gnode.r_epda_actual.r_epda_actual_1_0_0 import (
    Payload as REpdaActual100Payload,
)
from gwatn.types.ps_electricityprices_gnode.r_epda_actual.r_epda_actual_1_0_0 import (
    R_Epda_Actual_1_0_0,
)
...
class Payload(NamedTuple):
    ...
    MpAlias: str = "csv.epda.actual.1_0_0"
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "csv.epda.actual.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of 'csv.epda.actual.1_0_0', not {self.MpAlias}."
            )
        ...
        with open(price_file, newline="") as csvfile:
            ...
            for row in reader:
                ...
                if line_idx == 8:
                    ...
                    if header != "Day Ahead LMP Electricity Price (CurrencyUnit/MWh)":
                        raise Exception(
                            f"Csv 8,1 must be 'Day Ahead LMP Electricity Price (CurrencyUnit/MWh)', instead it is {header}"
                        )
