"""PayloadBase for r.eprt.actual.1_0_0"""
import datetime
...
class PayloadBase(NamedTuple):
    ...
    MpAlias: str = "r.eprt.actual.1_0_0"
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "r.eprt.actual.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of r.eprt.actual.1_0_0, not {self.MpAlias}."
            )
        ...
        if not isinstance(self.Core, GtEprtActual100):
            is_valid = False
            raise Exception(f"Make sure Core has type GtEprtActual100")
