"""PayloadBase for r.eprt.forecast.1_0_0"""
import datetime
...
class PayloadBase(NamedTuple):
    ...
    Prices: List[float]
    TimeSliceDurationMinutesList: List[int]
    IrlTimeUtc: Optional[str] = None
    WorldInstanceAlias: Optional[str] = None
    MpAlias: str = "r.eprt.forecast.1_0_0"
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "r.eprt.forecast.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of r.eprt.forecast.1_0_0, not {self.MpAlias}."
            )
        ...
        if not isinstance(self.TimeSliceDurationMinutesList, list):
            is_valid = False
            errors.append(
                f"TimeSliceDurationMinutesList {self.TimeSliceDurationMinutesList} must have type list."
            )
        else:
            for elt in self.TimeSliceDurationMinutesList:
                if not isinstance(elt, int):
                    is_valid = False
                    errors.append(
                        f"Elements of the list TimeSliceDurationMinutesList must have type int. Error with {elt}"
                    )
        if not isinstance(self.ForecastStartIso8601Utc, str):
            is_valid = False
            errors.append(
                f"ForecastStartIso8601Utc {self.ForecastStartIso8601Utc} must have type str."
            )
        if not property_format.is_recognized_iso8601_utc(self.ForecastStartIso8601Utc):
            is_valid = False
            errors.append(
                f"ForecastStartIso8601Utc {self.ForecastStartIso8601Utc} must have format RecognizedIso8601Utc."
            )
        ...
        if not isinstance(self.ForecastGeneratedIso8601Utc, str):
            is_valid = False
            errors.append(
                f"ForecastGeneratedIso8601Utc {self.ForecastGeneratedIso8601Utc} must have type str."
            )
        if not property_format.is_recognized_iso8601_utc(
            ...
            is_valid = False
            errors.append(
                f"ForecastGeneratedIso8601Utc {self.ForecastGeneratedIso8601Utc} must have format RecognizedIso8601Utc."
            )
