"""MessageMaker for r.epda.actual.1_0_0"""
...
from gwatn.types.ps_electricityprices_gnode.r_epda_actual.r_epda_actual_1_0_0_payload import (
    Payload,
)


class R_Epda_Actual_1_0_0(HackTypeBase):
    mp_alias = "r.epda.actual.1_0_0"
    routing_key_base = "ps.electricityprices.gnode"

    @classmethod
    def create_payload_from_camel_dict(cls, d: dict) -> Payload:
        if "MpAlias" not in d.keys():
            d["MpAlias"] = "r.epda.actual.1_0_0"
        if "Core" not in d.keys():
            raise SchemaError("Missing required 'Core' in r.epda.actual.1_0_0 message")
        GtCore = Gt_Epda_Actual_1_0_0.create_payload_from_camel_dict(d["Core"])
    ...
    def __init__(
        self, agent, core: GtEpdaActual100, irl_time_utc: Optional[str] = None
    ):
        ...
        if agent is None:
            raise Exception(
                f"Message protocol {R_Epda_Actual_1_0_0.mp_alias} must be generated by a message agent"
            )
        if agent == test_dummy.TEST_DUMMY_AGENT:
            ...
            if not agent.gni.g_node.is_ps:
                raise Exception(
                    f"Message protocol {R_Epda_Actual_1_0_0.mp_alias} must come from a Ps"
                )
        ...
        p = Payload(
            MpAlias=R_Epda_Actual_1_0_0.mp_alias,
