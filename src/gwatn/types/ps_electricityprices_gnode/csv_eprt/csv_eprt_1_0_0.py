...
from gwatn.types.ps_electricityprices_gnode.r_eprt.r_eprt_1_0_0 import (
    Payload as REprt100Payload,
)
from gwatn.types.ps_electricityprices_gnode.r_eprt.r_eprt_1_0_0 import R_Eprt_1_0_0
...
class Payload(NamedTuple):
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "csv.eprt.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of 'csv.eprt.1_0_0', not {self.MpAlias}."
            )
...
class Csv_Eprt_1_0_0:
    mp_alias = "csv.eprt.1_0_0"
        ...
        with open(real_time_electricity_price_csv, newline="") as csvfile:
            ...
            for row in reader:
                ...
                if line_idx == 12:
                    ...
                    if (
                        header
                        != "Real Time LMP Electricity Price (Currency Unit/MWh) and duration (hrs)"
                    ):
                        raise Exception(
                            f"Csv 12,1 must be 'Real Time LMP Electricity Price (Currency Unit/MWh) and duration (hrs)', instead it is {header}"
                        )
    ...
    def paired_rabbit_payload(self, agent) -> REprt100Payload:
