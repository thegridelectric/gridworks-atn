"""PayloadBase for r.eprt.forecast.cron.response.1_0_0"""
import datetime
...
class PayloadBase(NamedTuple):
    ...
    MpAlias: str = "r.eprt.forecast.cron.response.1_0_0"
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "r.eprt.forecast.cron.response.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of r.eprt.forecast.cron.response.1_0_0, not {self.MpAlias}."
            )
