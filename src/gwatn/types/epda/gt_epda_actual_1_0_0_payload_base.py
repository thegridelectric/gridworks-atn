"""PayloadBase for gt.epda.actual.1_0_0"""
import datetime
...
class PayloadBase(NamedTuple):
    ...
    MethodAlias: str  #
    Prices: List[float]
    MpAlias: str = "gt.epda.actual.1_0_0"
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "gt.epda.actual.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of gt.epda.actual.1_0_0, not {self.MpAlias}."
            )
