"""MessageMaker for r.weather.forecast.cron.response.1_0_0"""
...
from gwatn.types.ws_forecast_gnode.r_weather_forecast_cron_response.r_weather_forecast_cron_response_1_0_0_payload import (
    Payload,
)


class R_Weather_Forecast_Cron_Response_1_0_0(HackTypeBase):
    mp_alias = "r.weather.forecast.cron.response.1_0_0"
    routing_key_base = "ws.forecast.gnode"

    @classmethod
    def create_payload_from_camel_dict(cls, d: dict) -> Payload:
        if "MpAlias" not in d.keys():
            d["MpAlias"] = "r.weather.forecast.cron.response.1_0_0"
    ...
    def __init__(self, agent, to_g_node_alias: str, binding_key: str):
        ...
        if agent is None:
            raise Exception(
                f"Message protocol {R_Weather_Forecast_Cron_Response_1_0_0.mp_alias} must be generated by a message agent"
            )
        if agent == test_dummy.TEST_DUMMY_AGENT:
            ...
            if not agent.gni.g_node.is_ws:
                raise Exception(
                    f"Message protocol {R_Weather_Forecast_Cron_Response_1_0_0.mp_alias} must come from a Ws"
                )
        ...
        p = Payload(
            MpAlias=R_Weather_Forecast_Cron_Response_1_0_0.mp_alias,
