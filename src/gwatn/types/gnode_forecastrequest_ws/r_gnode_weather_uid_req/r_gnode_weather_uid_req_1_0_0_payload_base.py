"""PayloadBase for r.gnode.weather.uid.req.1_0_0"""
import datetime
...
class PayloadBase(NamedTuple):
    ...
    MpAlias: str = "r.gnode.weather.uid.req.1_0_0"
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "r.gnode.weather.uid.req.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of r.gnode.weather.uid.req.1_0_0, not {self.MpAlias}."
            )
        ...
        if not isinstance(self.ReturnMpAlias, str):
            is_valid = False
            errors.append(f"ReturnMpAlias {self.ReturnMpAlias} must have type str.")
