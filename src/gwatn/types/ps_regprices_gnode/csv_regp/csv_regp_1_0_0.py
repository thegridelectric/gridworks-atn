...
from gwatn.types.ps_regprices_gnode.r_regp.r_regp_1_0_0 import Payload as Regp100Payload
from gwatn.types.ps_regprices_gnode.r_regp.r_regp_1_0_0 import R_Regp_1_0_0
...
class Payload(NamedTuple):
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "csv.regp.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of 'csv.regp.1_0_0', not {self.MpAlias}."
            )
...
class Csv_Regp_1_1_0:
    mp_alias = "csv.regp.1_0_0"
    ...
    def __init__(self, reg_price_csv):
        self.errors = []
        self.payload = None
        first_data_row = 13
        prices = np.array([])
        slice_duration_minutes = np.array([])
        with open(reg_price_csv, newline="") as csvfile:
            ...
            for row in reader:
                ...
                if line_idx == 12:
                    ...
                    if (
                        header
                        != "Regulation Price (Currency Unit/MWh) and duration (hrs)"
                    ):
                        raise Exception(
                            f"Csv 12,1 must be 'Regulation Price (Currency Unit/MWh) and duration (hrs)'"
                        )
        ...
        if is_valid is False:
            ...
            print(errors)
            raise SchemaError(
                f"Failed to create payload due to these errors:{errors}. Input file is {reg_price_csv}"
            )
        else:
            self.payload = p

    def paired_rabbit_payload(self, agent) -> Regp100Payload:
