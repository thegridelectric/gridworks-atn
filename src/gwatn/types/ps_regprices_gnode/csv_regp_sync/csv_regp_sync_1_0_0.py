...
class Payload(NamedTuple):
    ...
    def passes_derived_validations(self) -> Tuple[bool, Optional[List[str]]]:
        is_valid = True
        errors = []
        if self.MpAlias != "csv.regp.sync.1_0_0":
            is_valid = False
            errors.append(
                f"Payload requires MpAlias of 'csv.regp.sync.1_0_0', not {self.MpAlias}."
            )
...
class Csv_Regp_Sync_1_0_0:
    mp_alias = "csv.regp.sync.1_0_0"
    ...
    def __init__(self, reg_price_csv):
        ...
        with open(reg_price_csv, newline="") as csvfile:
            ...
            for row in reader:
                ...
                if line_idx == 13:
                    ...
                    if header != "Regulation Price (Currency Unit/MWh)":
                        raise Exception(
                            f"Csv 13,1 must be 'Regulation Price (Currency Unit/MWh)', instead it is {header}"
                        )
        ...
        if is_valid is False:
            ...
            print(errors)
            raise SchemaError(
                f"Failed to create payload due to these errors:{errors}. Input file: {reg_price_csv} "
            )
        else:
            self.payload = p

    def paired_rabbit_payload(self, agent) -> RegpSync100Payload:
