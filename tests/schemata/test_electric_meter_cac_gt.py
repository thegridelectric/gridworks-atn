...
from gwatn.schemata import ElectricMeterCacGt_Maker as Maker
...
def test_electric_meter_cac_gt_generated() -> None:
    d = {
        ...
        "HasInternalCurrentMeasurement": False,
        "UpdatePeriodMs": 1000,
        "DefaultBaud": 9600,
    ...
    t = Maker(
        ...
        max_number_of_cts=gtuple.MaxNumberOfCts,
        local_comm_interface=gtuple.LocalCommInterface,
        has_internal_current_measurement=gtuple.HasInternalCurrentMeasurement,
    ...
    d2 = dict(d)
    del d2["HasInternalCurrentMeasurement"]
    with pytest.raises(SchemaError):
        Maker.dict_to_tuple(d2)
    ...
    d2 = dict(d, HasInternalCurrentMeasurement="this is not a boolean")
    with pytest.raises(ValidationError):
        Maker.dict_to_tuple(d2)
